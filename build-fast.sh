...
check_fast_config()
{
    ...
    grep -q 'atmel_pio4_set_pio_output(AT91_PIO_PORTC, 28, 0);' \
        "$ROOT/board/atmel/sama5d27_nextgen/sama5d27_nextgen.c" || {
        echo "error: fast U-Boot must explicitly force the NextGen backlight off" >&2
        exit 1
    }
    grep -q 'lcd-backlight-off-hog' "$ROOT/arch/arm/dts/sama5d27_nextgen.dts" || {
        echo "error: fast U-Boot lost the NextGen backlight GPIO hog" >&2
        exit 1
    }
